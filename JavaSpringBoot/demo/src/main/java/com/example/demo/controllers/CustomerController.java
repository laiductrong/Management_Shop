...
public class CustomerController {
    ...
    //login
    @GetMapping("/getUser/{phoneNumber}&{name}")
    public @ResponseBody
    ReposiveObject getUser(@PathVariable String phoneNumber, @PathVariable String name){
        List<Customer> foundCustomer = customerRepos.findByPhonenumberAndName(phoneNumber, name);
        return foundCustomer.size()>0? new ReposiveObject("oke", "login success", foundCustomer): new ReposiveObject("fail", "can't find customer", "");
    }
